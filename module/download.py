...
import requests
from module.paths import RAW, LOGS
...
DEFAULT_OUTPUT_DIR = RAW / "soiltest/strength"
DEFAULT_METADATA_DIR = RAW / "soiltest/metadata"
...
def download_records(
    ...
    for index, record in enumerate(records, start=1):
        ...
        row["xml_url"] = xml_url
        if row.get("download_status") in {"success", "skipped"} and destination.is_file() and destination.stat().st_size > 0:
